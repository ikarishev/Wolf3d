...
/*   Updated: 2018/09/06 14:46:39 by ikarishe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_line(t_info *info)
{
	...
	if (info->ray->side == 0)
		info->line->wall_x = info->player->y_loc + ((info->player->y) -
				(int)(info->player->y)) + info->ray->prep_wall_dist *
			info->ray->dir_y;
	else
		info->line->wall_x = info->player->x_loc + ((info->player->x) -
				(int)(info->player->x)) + info->ray->prep_wall_dist *
			info->ray->dir_x;
...
void	set_ray(t_info *info, int x, double *cam_x)
{
	*cam_x = 2 * x / (double)WIN_WIDTH - 1;
	info->ray->pos_x = info->player->x_loc + ((info->player->x) - (int)(info->player->x)) ;
	info->ray->pos_y = info->player->y_loc + ((info->player->y) - (int)(info->player->y)) ;
